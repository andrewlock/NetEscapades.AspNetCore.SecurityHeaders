...
{
    private static IIncrementalGenerator[] GetSourceGenerators() => [new ContentSecurityPolicyGenerator()];

    private static readonly string[] GeneratedAttributeFileNames = [];

    public static string EmbeddedAttribute { get; }
        = LoadEmbeddedResource("SourceGenerator.Test.CspMixinAttribute.cs");
    ...
    public static (ImmutableArray<Diagnostic> Diagnostics, string Output) GetGeneratedOutput(IIncrementalGenerator generator, Options opts)
    {
        var (diagnostics, trees) =
            GetGeneratedTrees([generator], opts, opts.Stages ?? GetTrackingNames<TrackingNames>());
